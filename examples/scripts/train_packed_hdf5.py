"""Generate packed HDF5 data, read it back, and train a small classifier.

The defaults are intentionally small and CPU-friendly. Run from the repository
root with::

    python examples/scripts/train_packed_hdf5.py
"""
...
def write_dataset(root: Path, sample_count: int, batch_size: int) -> None:
    """Generate transformed Signals and write them using packed HDF5."""
    generators = [
...
def make_loaders(
    ...
    """Reopen packed HDF5 and create deterministic data splits."""
...
def train(root: Path, sample_count: int, epochs: int, batch_size: int) -> None:
    """Write, reopen, train, and evaluate the packed HDF5 dataset."""
    torch.manual_seed(42)
    print(f"Writing {sample_count} samples to packed HDF5 at {root}")
...
def main() -> None:
    """Run the complete packed HDF5 training pipeline."""
    args = parse_args()
    root_context = (
        nullcontext(args.root)
        if args.root is not None
        else TemporaryDirectory(prefix="torchsig-packed-training-")
    )
