"""Generate homogeneous HDF5 data, read it back, and train a classifier.

The defaults are intentionally small and CPU-friendly. Run from the repository
root with::

    python examples/scripts/train_homogeneous_hdf5.py
"""
...
def parse_args() -> argparse.Namespace:
    """Parse generation and training options."""
...
def write_dataset(root: Path, sample_count: int, batch_size: int) -> None:
    """Generate fixed-shape Signals and write them using homogeneous HDF5."""
    generators = [
...
def make_loaders(
    ...
    """Reopen homogeneous HDF5 and create deterministic data splits."""
...
def train(root: Path, sample_count: int, epochs: int, batch_size: int) -> None:
    """Write, reopen, train, and evaluate the homogeneous HDF5 dataset."""
    torch.manual_seed(42)
    print(f"Writing {sample_count} samples to homogeneous HDF5 at {root}")
    write_dataset(root, sample_count, batch_size)
    train_loader, validation_loader, test_loader, dataset = make_loaders(
        root, sample_count, batch_size
    )
        ...
        for epoch in range(1, epochs + 1):
            ...
            for inputs, targets in train_loader:
                ...
                loss = nn.functional.cross_entropy(
                    model(inputs.float()), targets.long()
                )
                loss.backward()
                optimizer.step()
...
def main() -> None:
    """Run the complete homogeneous HDF5 training pipeline."""
    args = parse_args()
    root_context = (
        nullcontext(args.root)
        if args.root is not None
        else TemporaryDirectory(prefix="torchsig-homogeneous-training-")
    )
    with root_context as root:
        train(Path(root), args.samples, args.epochs, args.batch_size)
