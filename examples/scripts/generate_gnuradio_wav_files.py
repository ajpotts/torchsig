...
stereo-Q WAV dataset (BPSK, QPSK, 8-PSK) and the required side-car files.
...
Usage (in a Jupyter notebook) ------------------------------------------------
    ...
    gen.generate()                 # creates the whole data set
...
class IQDatasetGenerator:
    ...
    # --------------------------------------------------------------
    def generate_one(self, mod_name: str, snr_db: float, wav_path: str) -> None:
        ...
        wavfile.write(wav_path, self.audio_rate,
    ...
    def generate(self) -> None:
        ...
        for mod_name in self.mod_names:
            ...
            for snr in self.snr_db:
                fname = f"{mod_name}_{snr:+03.0f}dB_seed{self.seed}.wav"
                wav_path = out_dir / fname

                if not wav_path.is_file():
                    print(f"Generating {wav_path} …")
                    self.generate_one(mod_name, snr, str(wav_path))
                else:
                    print(f"File already exists: {wav_path}")
    ...
    def plot_example(
        ...
        wav_path = self.root / mod / f"{mod}_{snr:+03.0f}dB_seed{self.seed}.wav"
        if not wav_path.is_file():
            raise FileNotFoundError(f"Requested file not found: {wav_path}")

        sr, data = wavfile.read(str(wav_path))

        # The file is stored as float32 in the range [-1, +1] -- no extra scaling needed.
        if data.dtype == np.int16:
            ...
            raise TypeError(f"Unsupported sample dtype: {data.dtype}")
...
def plot_iq(wav_path: str) -> None:
    ...
    # --------------------------------------------------------------
    # Load the WAV -- scipy.io.wavfile returns the raw samples *exactly* as
    # they are stored in the file (int16 for PCM, float32 for float WAV).
    # --------------------------------------------------------------
    sr, data = wavfile.read(wav_path)        # shape = (N, 2)

    if data.ndim != 2 or data.shape[1] != 2:
        raise ValueError("Expected a stereo (2-channel) wav containing I & Q.")

    # --------------------------------------------------------------
    # Detect whether we got int16 or float32 and scale accordingly.
    # --------------------------------------------------------------
    if data.dtype == np.int16:
        # PCM -- need to map to [-1, 1]
        i = data[:, 0].astype(np.float32) / 32767.0
        q = data[:, 1].astype(np.float32) / 32767.0
    elif data.dtype == np.float32:
        # Already normalized -- no extra scaling.
        i = data[:, 0]
        q = data[:, 1]
    else:
