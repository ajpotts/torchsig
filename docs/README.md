...
classes and parameters retain their existing randomized behavior. The initial
schema supports fixed values; currently, QPSK pulse-shaping rolloff is the
supported generator parameter.
...
`alpha` must be a real number strictly between 0 and 1. A configured value
selects SRRC pulse shaping for QPSK and is recorded under
