...
def test_key_lookup_adds_note_to_missing_key_error():
    obj = HierarchicalMetadataObject()

    with pytest.raises(MetadataAttributeError) as exc_info:
        obj.key_lookup("missing")

    assert exc_info.value.__notes__ == ["key missing: 'missing'; "]


def test_attribute_lookup_adds_note_to_missing_key_error():
    obj = HierarchicalMetadataObject()
