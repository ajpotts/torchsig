...
def test_metadata_debug_formatter_formats_operation_record(caplog):
    ...
    assert formatted == (
        "session=session sample=7 worker=2 stage=generate event=lookup "
        "key=field source=local depth=0 value='value'"
    )
...
def test_metadata_debug_formatter_handles_disabled_values_and_summary(caplog):
    ...
    assert formatted_summary == (
        "metadata debug summary: emitted=1 suppressed=0 filtered=0"
    )
