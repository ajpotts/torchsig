...
def multistage_polyphase_resampler_actual_rate(resample_rate: float) -> float:
    ...
    if resample_rate > 1:
        ...
        if fractional_rate > 1:
            up_rate = 10_000
            down_rate = int(np.ceil(up_rate / fractional_rate))
    ...
    if resample_rate < 1:
        ...
        if fractional_rate > 1:
            ...
            up_rate = 10_000
            down_rate = int(np.ceil(up_rate / requested_fractional_rate))
...
def polyphase_decimator(input_signal: np.ndarray, decimation_rate: int) -> np.ndarray:
    ...
    length_off_by_one_floor_boolean = decimate_out.shape[-1] == int(
        np.floor(fractional_length) - 1
    )
    length_off_by_one_ceil_boolean = decimate_out.shape[-1] == int(
